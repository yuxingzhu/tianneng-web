  ...
  methods: {
    getChart(chartData) {
      ...
      chartData && chartData.map(item => {
        const name = item ? Object.keys(item) : ''
        const vaule = item ? item[Object.keys(item)].deviceCount : 0
        const percent = item ? item[Object.keys(item)].percent : '0'
        data.push({
          name: name,
          value: vaule,
          percent: percent
        ...
        series: [
          {
            ...
            label: {
              ...
              formatter: function(params) {
                let newParamsName = ''
                if (params.name[0].length > 6) { // 大于六个字符就显示...
                  newParamsName = params.name[0].substring(0, 6) + '...'
                } else {
                  newParamsName = params.name[0]
                }
